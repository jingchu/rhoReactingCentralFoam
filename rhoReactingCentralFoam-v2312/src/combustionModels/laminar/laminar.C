...
template<class ReactionThermo>
Foam::tmp<Foam::volScalarField>
Foam::combustionModels::laminar<ReactionThermo>::R2( volScalarField& Y ) const
{
    ...
    (
        ...
        (
            IOobject
            (
                this->thermo().phasePropertyName( typeName + "_Su" ),
    ...
    tSu.ref().ref() = this->chemistryPtr_->RR(specieI);
    }
    return tSu;
}
